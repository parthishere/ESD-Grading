...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderCriteria(criteria) {
        ...
        criteria.forEach(criterion => {
            ...
            // Create cells for criterion name and radio options
            row.innerHTML = `
                <td>${criterion.name}</td>
                <td class="text-center"><input type="radio" name="criteria_${criterion.id}" value="0"></td>
                <td class="text-center"><input type="radio" name="criteria_${criterion.id}" value="1"></td>
                <td class="text-center"><input type="radio" name="criteria_${criterion.id}" value="2" checked></td>
                <td class="text-center"><input type="radio" name="criteria_${criterion.id}" value="3"></td>
                <td class="text-center"><input type="radio" name="criteria_${criterion.id}" value="4"></td>
            `;
    ...
    function checkExistingSignoff(partId) {
        fetch(`/api/get-signoff-details/?student_id=${currentStudent.studentId}&part_id=${partId}`)
            .then(response => response.json())
            .then(data => {
                if (data.found) {
                    ...
                    document.querySelector(`input[name="overall_score"][value="${scoreValue}"]`).checked = true;
                    
                    // Set criteria scores
                    data.quality_scores.forEach(score => {
                        const scoreValue = calculateRadioValue(score.score, score.criteria__max_points);
                        const radio = document.querySelector(`input[name="criteria_${score.criteria_id}"][value="${scoreValue}"]`);
                        if (radio) {
                            radio.checked = true;
                        }
                    });
    ...
    function submitSignoff(e) {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                const statusBadge = document.getElementById(`status-${partIdInput.value}`);
                if (statusBadge) {
                    const status = data.status || getStatusFromOverallScore(formData.get('overall_score'));
    ...
    function resetFormInputs() {
        ...
        document.querySelector('input[name="overall_score"][value="2"]').checked = true;
